...
rowlist = []
qry = ("SELECT se.loc as loc, se.gene as gene, se.gt as gt, clin.dn as dn, an.rs as rs, clin.sig as sig, clin.vartype as clinvartype, an.pred as anpred, an.vartype as anvartype, se.pred as sepred, se.vartype as sevartype FROM se JOIN an on se.loc = an.loc LEFT OUTER JOIN clin on se.loc=clin.loc WHERE se.gene = %s ")
